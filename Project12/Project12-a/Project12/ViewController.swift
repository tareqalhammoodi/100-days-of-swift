...
class ViewController: UICollectionViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        if let savedData = defaults.object(forKey: "elementsData") as? Data {
            if let decodedData = try? NSKeyedUnarchiver.unarchiveTopLevelObjectWithData(savedData) as? [Element] {
                elements = decodedData
            }
        }
    }
    ...
    func saveData() {
        if let savedData = try? NSKeyedArchiver.archivedData(withRootObject: elements, requiringSecureCoding: false) {
            let defaults = UserDefaults.standard
